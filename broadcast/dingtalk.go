...
func (d *DingTalk) Broadcast(labels []string, symbol, direction, price, amount, total, profit string) {
	var title string
	switch direction {
	case Buy:
		title = "买入"
	...
	msg := fmt.Sprintf(`[%s]
[%s] [%s]
成交均价 %s, 成交量 %s, 成交额 %s, 利润 %s`, title, strings.Join(labels, "] ["), symbol, price, amount, total, profit)
	go func() {
